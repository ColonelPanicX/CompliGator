"""CISA Emergency Directives (ED) downloader.

CISA EDs are published as HTML pages at cisa.gov/news-events/directives/emergency-directive-*.
...
3. Curated KNOWN_URLS list — last resort if the index itself cannot be scraped;
   individual ED pages are still publicly accessible even when the index is blocked
"""
...
KNOWN_URLS_VERIFIED = "2026-03-05"

# Direct ED detail page URLs. Used when the index page cannot be scraped.
# Populate and update after verifying against SOURCE_URL.
KNOWN_URLS: list[str] = []
...
def _parse_ed_links(html: str) -> list[tuple[str, str]]:
    """Return list of (filename, full_url) for all ED detail pages on the index."""
...
def _write_known_urls_file(dest: Path) -> None:
    ...
    lines = [
        f"# CISA ED known fallback URLs — last verified {KNOWN_URLS_VERIFIED}",
...
def _playwright_download_pages(
    ...
    """Fetch ED HTML pages via Playwright. Returns (downloaded, skipped, errors)."""
    downloaded: list[str] = []
    skipped: list[str] = []
...
def _download_pages(
    ...
    """Download ED HTML pages: plain requests first, Playwright fallback for failures."""
    result = DownloadResult(framework="cisa-ed")
...
def run(
    ...
    if not links:
        result = DownloadResult(framework="cisa-ed")
        result.errors.append(("", f"Failed to fetch CISA emergency directives index: {SOURCE_URL}"))
        return result
    ...
    if dry_run:
        ...
        if used_known_urls:
            result.notices.append(
                ...
                f"See source-content/cisa-ed/_known-urls.txt for the full URL list."
            )
        return result
